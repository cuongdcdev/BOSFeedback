...
function bos_new_feedback($params)
{
    ...
    $term_name =  isset($cmt["project_name"]) ? strtolower(strip_tags(trim($cmt["project_name"]))) : "uncategorized";
    ...
    if (!is_array($parent_term)) {
        $parent_term_id = isset($parent_term) ? $parent_term['term_id'] : ''; // get numeric term id
    ...
    if (!is_wp_error($pid)) {
        wp_set_post_terms($pid, $term_name, 'bos_fb_tax'); 
